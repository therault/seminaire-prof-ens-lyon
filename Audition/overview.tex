...
%%        \only<4>{(Old Research Topics)}
%%        \only<5>{(Funding Projects)}
%%        \only<6>{(software development and dissemination)}}
...
\begin{frame}
  \frametitle{Three Eras of High Performance Computing Programming Paradigms}

  \begin{columns}
    \begin{column}{.5\textwidth}
      \resizebox{!}{.9\textheight}{
          \begin{tikzpicture}[scale=1.75]
            \draw[ultra thick,-latex] (0,0) -- (0,-7.5);
            \foreach \y in {196,197,...,203}{
              \draw (-.1,196-\y) -- (.1,196-\y);
              \node[align=right,anchor=east] at (-.1,196-\y) {\pgfmathprintnumber{\y}0};
            }

            \node[align=right,anchor=east] at (-.1,-0.67) {1KFlop/s (1967)};   %1967
            \node[align=right,anchor=east] at (-.1,-2.3)  {1GFlop/s (1983)};   %1983
            \node[align=right,anchor=east] at (-.1,-3.7)  {1TFlop/s (1997)};   %1997
            \node[align=right,anchor=east] at (-.1,-4.8)  {1PFlop/s (2008)};   %2008
            \node[align=right,anchor=east] at (-.1,-6.2)  {1EFlop/s (2022)};   %2024
            
            \fill[rounded corners=10pt,fill=black,fill opacity=0.1] (0.5,0) rectangle node[midway,align=center,color=black,opacity=1,rotate=90] {Dennard scaling (CPU Freq. $\nearrow$)} (0.8,-5);
            \fill[rounded corners=10pt,fill=black,fill opacity=0.1] (0.9,-1) rectangle node[midway,align=center,color=black,opacity=1,rotate=90] {Vector processing (amount of data / instruction $\nearrow$)} (1.2,-7);
            \fill[rounded corners=10pt,fill=black,fill opacity=0.1] (1.3,-2) rectangle node[midway,align=center,color=black,opacity=1,rotate=90] {Symmetric Multiprocessor (\# CPU $\nearrow$)} (1.6,-7);
            \fill[rounded corners=10pt,fill=black,fill opacity=0.1] (1.7,-3) rectangle node[midway,align=center,color=black,opacity=1,rotate=90] {Clusters (\# nodes $\nearrow$)} (2,-7);
            \fill[rounded corners=10pt,fill=black,fill opacity=0.1] (2.1,-5) rectangle node[midway,color=black,opacity=1,rotate=90] {GPUs (\# GPU $\nearrow$)} (2.4,-7);

            \fill[white,path fading=north] (0.5,-6.4) rectangle (4,-7); 
            \fill[white,path fading=north] (0.5,-4.3) rectangle (0.8,-5); 

	        \draw[ultra thick,dashed] (0.5,0) -- (2.5,0);
            \draw[ultra thick,dashed] (1.7,-3) -- (2.5,-3);
            \draw[ultra thick,latex-latex] (2.5,0) -- (2.5,-3) node[midway,align=left,anchor=west] {\Large 'Free Lunch' Era};

            \draw[ultra thick,dashed] (2.1,-5) -- (2.5,-5);
            \draw[ultra thick,latex-latex] (2.5,-3) -- (2.5,-5) node[midway,align=left,anchor=west] {\Large Cluster Era};

            \draw[ultra thick,latex-] (2.5,-5) -- (2.5,-6.4) node[midway,align=left,anchor=west] {\Large Hybrid Era};
	        \draw[ultra thick,dashed,-latex] (2.5,-6.4) -- (2.5,-7);
            
            \draw[->, >=latex, green!20!white, line width=15pt] (4.25,-3) to node[black,sloped,rotate=180]{Message Passing (MPI)} (4.25,-7);
            \draw[->, >=latex, blue!20!white, line width=15pt] (4.75,-4) to node[black,sloped,rotate=180]{Fault Tolerance} (4.75,-7);
            \draw[->, >=latex, brown!20!white, line width=15pt] (5.25,-5) to node[black,sloped,rotate=180]{Task Systems} (5.25,-7);

            \fill[white,path fading=south] (4,-3) rectangle (4.5,-4);
            \fill[white,path fading=south] (4.5,-4) rectangle (5,-5);
            \fill[white,path fading=south] (5,-5) rectangle (5.5,-6); 
          \end{tikzpicture}
      }
    \end{column}\hspace*{-1.5cm}\begin{column}{.6\textwidth}
      \small
      \begin{description}
      \item['Free Lunch'] Hardware improvements (frequency increase), and compiler support (vector computing) drive performance improvement
      \item[Cluster] Multiprocessor then multinode programming requires \textcolor{green!50!black}{rewriting the programs for parallelism}. \textcolor{blue}{As node number increases, failures happen and can be tolerated}
      \item[Hybrid] Nodes feature GPUs; NUMA effects become significant; network becomes multihop $\Rightarrow$ \textbf{Large heterogeneity of computing and communication costs}. \textcolor{brown}{Runtime systems for dynamic task management becomes necessary}
      \end{description}
    \end{column}
  \end{columns}
